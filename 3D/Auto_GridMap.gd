...
#@export var mesh_library_3d = mesh_library_3d as MeshLibrary # ( Resource )
@export var mesh_library_3d : MeshLibrary # ( Resource )
...
func set_cell_item(position: Vector3i, item: int, orientation: int = 0):
	super.set_cell_item(position, item)
...
func update_sub_cell( x, y, z ):
	if get_main_cell_item( x, y, z ) == -1:
		sub_gridmap.set_cell_item(Vector3(x, y, z), -1)
		return
	...
	if meshes.has( mask ):
		...
		sub_gridmap.set_cell_item(Vector3(x, y, z), mesh, orientation)
	else:
		sub_gridmap.set_cell_item(Vector3(x, y, z), -1)
